...
export class CinemaManager {
  ...
  pay(bookingId: string, card: string | null): string {
    ...
    if (!LegacyPaymentGateway.charge(card, b[6] as number)) {
      this.notifications.paymentDeclined(b[1] as string, bookingId);
  ...
  cancel(bookingId: string): string {
    ...
    this.notifications.bookingCancelled(b[1] as string, bookingId, refund);
  ...
  expireOld(): void {
    ...
    for (const [key, b] of LegacyDb.BOOKINGS) {
      if ((b[7] as number) === 0
        ...
        this.notifications.bookingExpired(b[1] as string, key);
      }
    }
  }
