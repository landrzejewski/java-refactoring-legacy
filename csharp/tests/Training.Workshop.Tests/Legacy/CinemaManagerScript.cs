using Training.Workshop.Legacy;
...
/// </summary>
internal static class CinemaManagerScript
{
    internal static string Run()
    {
        LegacyDb.Clear();
        var log = new List<string>();
        var cinema = new CinemaManager();
        ...
        try
        {
            ...
            log.AddRange(LegacyMailer.Sent);
            log.AddRange(LegacyPaymentGateway.Charges);
            return string.Join("\n", log) + "\n";
        }
        finally
        {
            CinemaManager.Clock = () => DateTime.Now;
            LegacyDb.Clear();
        }
    }
}
