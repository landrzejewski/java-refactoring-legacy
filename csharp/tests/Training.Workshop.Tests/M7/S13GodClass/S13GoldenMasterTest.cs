...
/// </summary>
[Collection("Legacy")]
public sealed class S13GoldenMasterTest
{
    // pusta linia przed zamknięciem: tekst kończy się znakiem nowej linii, jak text block w Javie
    internal const string Approved = """
        book: B1 B2 B3 B4 B5
    ...
    private static readonly Scene<string, string> Scene = Support.Scene.Variants<string, string>()
        .Variant("start", day => S13Script.Run(new Start.Driver()))
        .Variant("step1", day => S13Script.Run(new Step1.Driver()))
        .Variant("step2", day => S13Script.Run(new Step2.Driver()))
        .Variant("step3", day => S13Script.Run(new Step3.Driver()))
        .Variant("step4", day => S13Script.Run(new Step4.Driver()))
    ...
    public void EveryStepOfTheCampaignKeepsTheApprovedBehaviour(string test) => Scene.Run(test);
}
