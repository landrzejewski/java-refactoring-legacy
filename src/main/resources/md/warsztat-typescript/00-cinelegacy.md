# Warsztat CineLegacy (TypeScript) - praktyczne sceny do modułów 3-8
...
Warsztat ma trzy wersje o tych samych scenach i krokach: [Java](../warsztat/00-cinelegacy.html), [C#](../warsztat-csharp/00-cinelegacy.html) i TypeScript (ta strona). Skrypt prowadzącego wybiera wersję opcją `--lang java|cs|ts`.

Wersja TypeScript ma te same reguły biznesowe i oczekiwane wyniki co wersja Java. Tam, gdzie mechanizm języka jest inny, przewodnik ma w scenie akapit **Różnica względem Javy**.

## Jak zbudowana jest scena

```text
typescript/src/workshop/
    shared/Money.ts, time.ts        wspólne typy (kwoty, odpowiedniki java.time) - nie refaktoryzujemy ich
    legacy/                         stary system: CinemaManager (God Class), LegacyDb, LegacyMailer, LegacyPaymentGateway
    m6/s08_state/
        Payments.ts, Status.ts      stabilne typy kontraktu sceny (opcjonalnie)
        start/                      kod wyjściowy - na nim pracujemy na żywo
        step1/ step2/ ...           gotowe snapshoty po każdym kroku przewodnika
typescript/test/workshop/
    legacy/CinemaManagerGoldenMasterTest.test.ts
    support/scene.ts                te same przypadki dla start i każdego kroku
    m6/s08_state/S08EquivalenceTest.test.ts ...
```
...
- `start` zawiera zapach albo pułapkę. Prowadzący otwiera go w VS Code i wykonuje ruchy opisane w przewodniku.
- `stepN` to kompletny stan kodu po kroku N. Pliki i klasy mają te same nazwy co w `start`, a kroki importują typy sceny ścieżką względną (`../Status.js`), więc krok można skopiować do `start` bez zmian. Ostatni krok jest rozwiązaniem wzorcowym.
...
```bash
scripts/warsztat.sh --lang ts list            # wszystkie sceny i ich kroki
scripts/warsztat.sh --lang ts list m6         # sceny jednego modułu
scripts/warsztat.sh --lang ts test m6/s08     # testy jednej sceny (vitest)
scripts/warsztat.sh --lang ts test m6         # testy całego modułu
scripts/warsztat.sh --lang ts diff m6/s08 1 2 # co dokładnie zmienia krok 2 (0 = start), w kolorach
scripts/warsztat.sh --lang ts diff m6/s08 1 2 --word  # zmiany podświetlone w obrębie linii
scripts/warsztat.sh --lang ts jump m6/s08 2   # skopiuj step2 do start - przeskok, gdy brakuje czasu
scripts/warsztat.sh --lang ts next            # następny krok do start (ostatnio używana scena)
scripts/warsztat.sh --lang ts prev            # krok wstecz
scripts/warsztat.sh --lang ts status          # który krok jest teraz w start
scripts/warsztat.sh --lang ts reset m6/s08    # przywróć start z repozytorium po pokazie
scripts/warsztat.sh html            # wygeneruj strony HTML z md/warsztat (Node.js)
```

> `reset` przywraca `start` z gita, więc pliki warsztatu muszą być w repozytorium (co najmniej `git add`). Przed zajęciami: `cd typescript && npm ci && npm run typecheck && npm test` (wszystko zielone) i `git status` (czysty `start`). Vitest nie sprawdza typów - gdy ruch w scenie opiera się na błędzie kompilacji, uruchom `npm run typecheck`.

Opcja `--lang ts` wybiera wersję TypeScript. Skrypt zapamiętuje ostatnio użyty język i scenę, więc `next`, `prev` i `status` działają potem bez argumentów.
...
3. `scripts/warsztat.sh --lang ts test mM/sNN`, czyli zielony test.
...
Diff jest kolorowy w terminalu (git diff). Jeśli zainstalujesz `delta` (`brew install git-delta`), skrypt użyje go automatycznie: dostaniesz podświetlanie składni TypeScript i numery linii, co dobrze wygląda na rzutniku.

W VS Code wygodnie jest uruchamiać test sceny z rozszerzenia Vitest (ikona przy `describe`) albo ze skryptu w terminalu obok edytora.
...
`legacy/CinemaManager` to "serce" starego systemu. Ma kody liczbowe i tekstowe, tablice `unknown[]` jako rekordy, statyczną "bazę", maile wysyłane z logiki biznesowej, `number` na pieniądze i zagnieżdżone warunki. Z niego wycięto większość scen. Ten sam kod jest też materiałem kampanii God Class (moduł 7) oraz Branch by Abstraction i Strangler Fig (moduł 8).
...
```bash
cd typescript && npx vitest run test/workshop/legacy
```

## Materiały

| Moduł | Przewodnik prowadzącego | Zadania dla uczestników | Katalog |
|---|---|---|---|
| 3. Zasady dobrego projektowania | [przewodnik](przewodnik/03-zasady-dobrego-projektowania.html) | [zadania](zadania/03-zasady-dobrego-projektowania.html) | `workshop/m3` |
| 4. Podstawowe refaktoryzacje | [przewodnik](przewodnik/04-podstawowe-refaktoryzacje.html) | [zadania](zadania/04-podstawowe-refaktoryzacje.html) | `workshop/m4` |
| 5. Refaktoryzacje hierarchii klas | [przewodnik](przewodnik/05-refaktoryzacje-hierarchii-klas.html) | [zadania](zadania/05-refaktoryzacje-hierarchii-klas.html) | `workshop/m5` |
| 6. Refaktoryzacje do wzorców projektowych | [przewodnik](przewodnik/06-refaktoryzacje-do-wzorcow-projektowych.html) | [zadania](zadania/06-refaktoryzacje-do-wzorcow-projektowych.html) | `workshop/m6` |
| 7. Zaawansowane refaktoryzacje | [przewodnik](przewodnik/07-zaawansowane-refaktoryzacje.html) | [zadania](zadania/07-zaawansowane-refaktoryzacje.html) | `workshop/m7` |
| 8. Strategie i dobre praktyki | [przewodnik](przewodnik/08-strategie-i-dobre-praktyki.html) | [zadania](zadania/08-strategie-i-dobre-praktyki.html) | `workshop/m8` |
...
| [m4/s01](przewodnik/04-podstawowe-refaktoryzacje.html#scena-s01-rename-nazwy-ktore-zyja-poza-kodem) | Rename - nazwy, które żyją poza kodem | Rename - cel i mechanika; Co ma pozostać niezmienione (refleksja, konfiguracja) | 3 |
...
| [m5/s12](przewodnik/05-refaktoryzacje-hierarchii-klas.html#scena-s12-generyki-wymazywanie-typow-i-brak-metod-bridge) | Generyki, wymazywanie typów i brak metod bridge | 2.5-2.8. Sygnatury po erasure i metody bridge; 10.2-10.3. Refleksja | 2 |
| [m5/s13](przewodnik/05-refaktoryzacje-hierarchii-klas.html#scena-s13-hierarchie-zamkniete-i-wyczerpujacy-switch) | Hierarchie zamknięte i wyczerpujący switch | 2.5-2.8. `sealed`, `permits`, `MatchException` | 3 |
...
| [m5/s15](przewodnik/05-refaktoryzacje-hierarchii-klas.html#scena-s15-zgodnosc-zbudowanego-kodu-refleksja-i-metadane) | Zgodność zbudowanego kodu, refleksja i metadane | 1.3. Warstwy zgodności; 10.2-10.3. Zgodność binarna, refleksja i adnotacje | 4 |
...
| [m8/s08](przewodnik/08-strategie-i-dobre-praktyki.html#scena-s08-bramka-kompilatora-opcje-strict-i-ostrzezenia-jako-bledy) | Bramka kompilatora - opcje strict i ostrzeżenia jako błędy | 5.2-5.3 Refaktoryzacje IDE i kompilator Javy 25; 5.6 Bramka kompilatora | 3 |
| [m8/s09](przewodnik/08-strategie-i-dobre-praktyki.html#scena-s09-codemod-na-api-kompilatora-typescript) | Codemod na API kompilatora TypeScript | 5.4-5.5 Analiza statyczna, formatowanie i automatyzacja (receptury) | 3 |
