# Warsztat CineLegacy - praktyczne sceny do modułów 3-8
...
```bash
scripts/warsztat.sh list            # wszystkie sceny i ich kroki
...
> `reset` przywraca `start` z gita, więc pliki warsztatu muszą być w repozytorium (co najmniej `git add`). Przed zajęciami: `mvn -q test` (wszystko zielone) i `git status` (czysty `start`).
...
| [m4/s01](przewodnik/04-podstawowe-refaktoryzacje.html#scena-s01-rename-nazwy-ktore-zyja-poza-java) | Rename - nazwy, które żyją poza Javą | Rename - cel i mechanika; Co ma pozostać niezmienione (refleksja, konfiguracja) | 3 |
...
| [m8/s08](przewodnik/08-strategie-i-dobre-praktyki.html#scena-s08-bramka-kompilatora-xlintall-werror) | Bramka kompilatora -Xlint:all -Werror | 5.2-5.3 Refaktoryzacje IDE i kompilator Javy 25; 5.6 Bramka kompilatora | 3 |
| [m8/s09](przewodnik/08-strategie-i-dobre-praktyki.html#scena-s09-codemod-na-compiler-tree-api) | Codemod na Compiler Tree API | 5.4-5.5 Analiza statyczna, formatowanie i automatyzacja (receptury) | 3 |
