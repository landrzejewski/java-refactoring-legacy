# Moduł 8. Strategie i dobre praktyki - warsztat CineLegacy (C#): zadania

Każda scena ma katalog (namespace) `Start` - na nim pracujesz. Po każdym ruchu uruchom test sceny (`scripts/warsztat.sh --lang cs test m8/sNN` albo w Rider z gutteru przy klasie testu lub z okna Unit Tests) i nie zaglądaj do katalogów `StepN` przed końcem zadania. Część testów opisuje stan startowy (np. "start wysyła dwa maile", "start narusza ADR") - gdy naprawisz `Start`, zmienią kolor na czerwony, a to znaczy, że zadanie jest zrobione. Testy czytające pliki źródłowe znajdują je same (pomocnik `SourceFiles`), więc katalog, z którego je uruchamiasz, nie ma znaczenia. Powrót do stanu wyjściowego: `scripts/warsztat.sh --lang cs reset m8/sNN`.

Kod leży w `csharp/src/Training.Workshop/M8/SNN.../Start`, testy w `csharp/tests/Training.Workshop.Tests/M8/SNN.../`.

## Scena s01. Branch by Abstraction na cenniku z CinemaManager
**Namespace:** `Training.Workshop.M8.S01BranchByAbstraction.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s01`
...
2. Wprowadź abstrakcję `ITicketPricing` zwracającą `Money` i schowaj za nią stary cennik.
...
**Podpowiedź:** po krokach 1 i 2 nic nie powinno się zmienić - to jeszcze refaktoryzacja. Konwersja `double` na `Money` tylko na granicy starego kodu (`new Money((decimal)total)`).
**Gotowe, gdy:** test równoważności jest zielony po każdym kroku (w kroku 3 dla obu ustawień przełącznika), a na końcu w namespace nie ma ani starego cennika, ani przełącznika.

## Scena s02. Strangler Fig - fasada, która przejmuje ścieżki
**Namespace:** `Training.Workshop.M8.S02StranglerFig.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s02`
...
**Zadanie:**
1. Postaw przed `LegacyCinema` fasadę implementującą `ICinemaApi`, która deleguje wszystko 1:1.
...
**Namespace:** `Training.Workshop.M8.S03ParallelRun.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s03`
...
**Namespace:** `Training.Workshop.M8.S04ShadowLimits.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s04`
...
**Namespace:** `Training.Workshop.M8.S05BoyScout.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s05`
...
2. Nie zmieniaj niczego, co widzi klient: kolejności miejsc, wielkości liter, linii z telefonem, formatu kwoty i daty.
...
**Namespace:** `Training.Workshop.M8.S06ReviewableSeries.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s06`
...
**Namespace:** `Training.Workshop.M8.S07Adr.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s07`
...
**Zadanie:**
1. Przeczytaj `ADR-0007-cennik-jako-czysty-modul.md` (katalog sceny `M8/S07Adr`) i sprawdź w teście, które reguły narusza `Start`.
2. Usuń zależność przestrzeni nazw `Pricing` od `Notification` bez zmiany wysyłanych maili.
3. Zamień kwoty w przestrzeni nazw `Pricing` na `Money`.
...
**Namespace:** `Training.Workshop.M8.S08CompilerGate.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s08`

**Zasada:** Bramka kompilatora (tu: Roslyn z włączonym nullable, wszystkimi falami ostrzeżeń i ostrzeżeniami jako błędami) zamienia ostrzeżenia w błędy builda. Ostrzeżenia usuwa się zmianą kodu, po jednej przyczynie na krok, a tłumienie jest wyjątkiem wąskim i uzasadnionym.

**Zadanie:**
1. Sprawdź, jakie identyfikatory ostrzeżeń (CS...) zgłasza `CompilerGate` dla `Start`.
2. Usuwaj je po jednej przyczynie na krok, uruchamiając test po każdym kroku.
3. Zachowaj zachowanie raportu, w tym to, że IMAX ma też dźwięk Dolby.
**Podpowiedź:** nie używaj `#pragma warning disable`, `[SuppressMessage]` ani nie dopisuj reguł do `.editorconfig` - każde ostrzeżenie da się tu usunąć zmianą kodu. Plik `M8/S08CompilerGate/.editorconfig` wycisza te ostrzeżenia tylko w buildzie projektu; `CompilerGate` go nie czyta.
...
**Namespace:** `Training.Workshop.M8.S09Codemod.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s09`
...
**Zadanie:**
1. Zastąp wyszukiwanie wyrażeniem regularnym wyszukiwaniem po drzewie składni (`CSharpSyntaxTree`, `InvocationExpressionSyntax`).
2. Przepisuj tylko dwa ostatnie argumenty wywołania na drzewie składni (`CSharpSyntaxRewriter`) i dopisz brakującą dyrektywę `using`.
3. Migruj wyłącznie wywołania przestarzałej metody `Cinema.BookingService.Book` - użyj informacji o typach (`SemanticModel`).
**Podpowiedź:** `SampleProject.TicketDesk` ma wywołanie w komentarzu, wywołanie rozbite na trzy linie i podobne wywołanie w `HotelService`. Sprawdź też, co się stanie po drugim uruchomieniu codemodu.
**Gotowe, gdy:** codemod znajduje linie 13 i 19, wynik kompiluje się bez ostrzeżeń razem z `SampleProject.Api`, a drugie uruchomienie niczego nie zmienia.

## Scena s10. Minimalna bramka jakości jako kod
**Namespace:** `Training.Workshop.M8.S10QualityGate.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s10`
...
**Zadanie:**
1. Zamień listę kontrolną z komentarza `QualityGate` na wykonywalne sprawdzenia, po jednym na krok.
2. Każdy wynik ma wskazywać plik i linię albo metodę.
3. Czysta próbka (`Sample/Clean`) musi przejść bramkę bez żadnego wyniku.
**Podpowiedź:** zacznij od najtańszego sprawdzenia (skan tekstu źródeł), kompilator i uruchamianie testów zostaw na koniec. Próbki i ich testy nie wchodzą do kompilacji projektu - kompilację i uruchomienie testów musi zrobić sama bramka (Roslyn w pamięci).
...
**Namespace:** `Training.Workshop.M8.S11StagedRollout.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s11`
...
**Podpowiedź:** przydział ma być taki sam po restarcie i na każdej maszynie - nie losuj i nie polegaj na `GetHashCode` (dla `string` w .NET zmienia się przy każdym starcie procesu). Stabilny skrót da np. `SHA256.HashData`.
...
**Namespace:** `Training.Workshop.M8.S12ExpandContract.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s12`
...
**Namespace:** `Training.Workshop.M8.S13LivingDocs.Start` · **Test:** `scripts/warsztat.sh --lang cs test m8/s13`
...
**Podpowiedź:** ścieżkę dokumentu wylicz z przestrzeni nazw klasy (`typeof(...).Namespace`) - wtedy generator zadziała też po skopiowaniu sceny. Biblioteka nie ma `Main`, więc generator wystaw jako metodę statyczną i wywołaj ją z katalogiem roboczym `csharp/`.
