# Moduł 3. Zasady dobrego projektowania - warsztat CineLegacy (C#): zadania

Każda scena to mały fragment systemu kina CineLegacy z jednym problemem projektowym. Pracujesz wyłącznie w przestrzeni nazw `Start` danej sceny (katalog `csharp/src/Training.Workshop/M3/SNN.../Start`). Po każdej zmianie uruchamiaj test sceny - ma pozostać zielony (o ile zadanie nie mówi inaczej). Nie zaglądaj do katalogów `Step1`, `Step2`, ... przed końcem - to rozwiązania wzorcowe. Po zadaniu możesz porównać swoje rozwiązanie: `scripts/warsztat.sh --lang cs diff m3/sNN 0 N`, a przywrócić stan wyjściowy: `scripts/warsztat.sh --lang cs reset m3/sNN`.

Zasady:

- Małe kroki: jedna refaktoryzacja, jeden przebieg testu. Używaj automatycznych refaktoryzacji Rider tam, gdzie się da (⌃T Refactor This, ⌥⌘M Extract Method, ⌥⌘N Inline, ⌘F6 Change Signature, ⌘⌦ Safe Delete, F6 Move).
...
- Test sceny uruchamiasz skryptem (`scripts/warsztat.sh --lang cs test m3/sNN`) albo w Rider z ikony w marginesie klasy testowej.
- Liczby domenowe: 2D 25.00, 3D 32.00, IMAX 40.00; zniżki STUDENT 25%, SENIOR 30%, CHILD 40%; seans przed 12:00 -5.00; VIP +10.00; okulary 3D +3.00; opłata rezerwacyjna online 2.00 za bilet; zwrot 100% / 50% / 0% minus 3.00. Kwoty to `decimal`.

## Scena s01. DRY - jedna wiedza, dwie reprezentacje
**Namespace:** `Training.Workshop.M3.S01DryKnowledge.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s01`
...
**Namespace:** `Training.Workshop.M3.S02Similarity.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s02`
...
**Zadanie:**
1. Oceń statyczną klasę `ServiceFee`: jaką wiedzę reprezentuje i kto jest właścicielem każdej z reguł?
...
**Podpowiedź:** tymczasowe powtórzenie kodu jest dozwolone - zacznij od Inline Method. Jeśli kompilator zgłosi CS1718 (porównanie stałej enuma z samą sobą), to wskazówka, którą gałąź usunąć w następnym kroku.
**Gotowe, gdy:** test zielony, `ServiceFee` nie istnieje, każda kwota jest stałą (`const decimal`) u swojego właściciela.

## Scena s03. Fałszywa abstrakcja z flagami
**Namespace:** `Training.Workshop.M3.S03FalseAbstraction.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s03`
...
**Zadanie:**
1. Usuń metodę `Pricing.Price` sterowaną flagami `bool`.
...
**Podpowiedź:** Inline Method (⌥⌘N), potem Inline Variable dla stałych argumentów i ⌥⏎ na warunkach zawsze prawdziwych lub fałszywych.
**Gotowe, gdy:** test zielony, żadna metoda nie przyjmuje flagi `bool`, której wywołujący nie potrzebuje, a `PassCounter` nie wie nic o formatach.

## Scena s04. DRY w testach i niezależna wyrocznia
**Namespace:** `Training.Workshop.M3.S04DryTests.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s04`
...
2. Spraw, żeby komunikat o błędzie mówił, który przypadek, jaka oczekiwana i jaka faktyczna cena (kwoty w formacie `0.00`, niezależnie od kultury).
...
**Namespace:** `Training.Workshop.M3.S05Kiss.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s05`
...
2. Zastąp wyrażenie regularne i sekwencję indeksów (`Enumerable.Range`) prostszym algorytmem.
...
**Gotowe, gdy:** test zielony, w klasie nie ma `System.Reflection`, `System.Text.RegularExpressions` ani atrybutów `[SuppressMessage]` uciszających "nieużywane" metody.

## Scena s06. YAGNI - silnik reguł dla dwóch reguł
**Namespace:** `Training.Workshop.M3.S06Yagni.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s06`
...
**Zadanie:**
1. Przejdź filtr decyzyjny: jakie aktualne wymaganie uzasadnia rejestr, konfigurację napisem, priorytety i kontekst `IDictionary<string, object>`?
2. Uprość `TicketPricer` w kilku krokach, zachowując publiczne `new TicketPricer()` i `Price(TicketQuote)`.
3. Zapisz w komentarzu XML (`/// <summary>`), czego YAGNI nie zabrania i co świadomie zostawiasz.
...
**Namespace:** `Training.Workshop.M3.S07Srp.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s07`
...
**Namespace:** `Training.Workshop.M3.S08Ocp.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s08`
...
**Podpowiedź:** formaty to dane, nie algorytmy. Enum C# nie niesie danych, więc rozważ `sealed class Format` ze statycznymi instancjami, prywatnym konstruktorem i właściwościami (ceną, okularami, etykietą) zamiast hierarchii klas. Etap pośredni - zwykły `enum` i `switch` bez gałęzi `_` - też jest wartościowy: brak obsługi nowej stałej to błąd kompilacji.
...
**Namespace:** `Training.Workshop.M3.S09Lsp.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s09`
...
**Podpowiedź:** raport potrzebuje tylko odczytu planu miejsc. Extract Interface, potem ręcznie Replace Inheritance with Delegation (Rider pomoże: ⌘N > Delegating Members).
**Gotowe, gdy:** test zielony, `ReadOnlyHall` nie rzuca `NotSupportedException`, `Hall` jest `sealed`, a próba przekazania sali archiwalnej do kasy nie kompiluje się.

## Scena s10. ISP z perspektywy klienta
**Namespace:** `Training.Workshop.M3.S10Isp.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s10`
...
**Zadanie:**
1. Dla każdego klienta (`CashDesk`, `RevenueReport`, `ScheduleBoard`) wypisz metody `ICinemaAdminService`, których naprawdę używa.
...
**Namespace:** `Training.Workshop.M3.S11Dip.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s11`
...
**Zadanie:**
1. Narysuj strzałki zależności (`using`) i wywołań między `App` i `Infra`.
2. Wstrzyknij zależność przez konstruktor i sprawdź, czy kierunek zależności się zmienił.
3. Wprowadź port nazwany potrzebą przypadku użycia i adapter w `Infra`. Graf składaj w statycznej klasie `Main`.

**Podpowiedź:** nie rób Extract Interface z `SmtpMailSender` - najpierw wydziel metodę, która mówi, czego potrzebuje polityka.
**Gotowe, gdy:** test zielony, żaden plik w `App` nie ma `using ...Infra`, a przypadek użycia da się przetestować ręcznym fake'iem portu zamiast SMTP.

## Scena s12. Clean Architecture - use case, dane na granicy, composition root
**Namespace:** `Training.Workshop.M3.S12CleanArchitecture.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s12`
...
3. Rozdziel kod na przestrzenie nazw `App` i `Adapter`, a składanie grafu przenieś do `CinemaApplication`.
4. Napisz test przypadku użycia (xUnit) na adapterach w pamięci, w tym przypadek: zapis się nie udał, więc nie ma powiadomienia.

**Podpowiedź:** kolejność "zapis, potem powiadomienie" to decyzja protokołu - niech będzie jawna w przypadku użycia.
**Gotowe, gdy:** test zielony, `App` nie używa `Adapter` ani `RowStore`/`Outbox`, kontroler tylko tłumaczy żądanie i odpowiedź, a `CinemaApplication` nie zawiera reguł biznesowych.

## Scena s13. Automatyczna ochrona granicy i diagnostyka spójności
**Namespace:** `Training.Workshop.M3.S13BoundaryCheck.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s13`
...
**Zadanie:**
1. Uruchom test `S13ArchitectureTest` w Rider i odczytaj z wyjścia testu naruszenia granicy oraz wynik LCOM4 dla `ScreeningService`.
2. Popraw spójność `ScreeningService`, a potem kierunek zależności, tak żeby przestrzeń nazw `Domain` nie używała `Adapter` ani `System.Data`.
3. Zaproponuj silniejszą bramkę niż skan dyrektyw `using` (np. NetArchTest / ArchUnitNET, osobny projekt domeny bez referencji do adapterów, analizator Roslyn) i porównaj koszt.
...
**Namespace:** `Training.Workshop.M3.S14ReversiblePattern.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s14`
...
**Podpowiedź:** po każdym kroku test równoważności dla modelu procentowego musi być zielony. Sygnały nadmiaru wzorca: jedna implementacja, słownik z jednym wpisem.
...
**Namespace:** `Training.Workshop.M3.S15Invariants.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s15`
...
3. Przenieś reguły poprawności do modelu tak, żeby nie dało się utworzyć złej rezerwacji żadną ścieżką - także wyrażeniem `with`.

**Podpowiedź:** najpierw rekord pozycyjny, potem rekord z jawnym konstruktorem ze strażnikami. C# nie ma kompaktowego konstruktora rekordu, a właściwości `init` pozwoliłyby ominąć strażników przez `with` - użyj właściwości tylko `{ get; }`.
...
**Namespace:** `Training.Workshop.M3.S16TemporalCoupling.Start` · **Test:** `scripts/warsztat.sh --lang cs test m3/s16`
...
**Zadanie:**
1. Zakomentuj jedno z wywołań `Select...` w `TicketDesk` i uruchom test - zobacz, gdzie i jak objawia się błąd (i dlaczego kompilator milczał mimo nullable). Cofnij zmianę.
...
**Podpowiedź:** Change Signature, potem Introduce Parameter Object (w Rider: Extract Class from Parameters) i `ArgumentNullException.ThrowIfNull` w konstruktorze.
